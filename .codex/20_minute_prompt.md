...
OpenGame reference direction: `docs/opengame_skills/` contains copied OpenGame Template Skill and Debug Skill docs for process inspiration only. Use the Template Skill idea for future TOEFL world-pack/content patterns, and use the Debug Skill idea for a local playtest/debug protocol that records recurring player-facing failures, root causes, verified fixes, and regression coverage. Do not import OpenGame, Node, Phaser, browser-game scaffolds, asset-generation pipelines, or new API-key requirements into this Python terminal RPG runtime.

Follow the repository as the source of truth:

1. Read `AGENTS.md`, `docs/ROADMAP.md`, `docs/QUALITY_GATES.md`, `docs/AUTOMATION_RUNBOOK.md`, and `docs/opengame_skills/README.md`.
...
6. Make the smallest vertical change that satisfies the task acceptance criteria. Do not do unrelated refactors, introduce OpenGame runtime dependencies, or start a second task.
