...
def test_learner_sentence_corpus_covers_vaccine_bench_visible_vaccine_definition_before_practice() -> None:
    target_case_id = "vaccine_bench_visible_vaccine_definition_before_practice"
    visible_vaccine_cases = [
        case
        for case in load_corpus()
    ...
    assert any(
        ...
        and case["expected_parser"]["action"] == "explain"
        and case["expected_parser"]["target"] == "vaccine"
        and case["expected_success"] is True
        and case["expected_room_id"] == "vaccine_bench"
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 1
        for case in visible_vaccine_cases
    )


def test_learner_sentence_corpus_covers_indirect_vaccine_bench_visible_vaccine_definition_before_practice() -> None:
    target_case_id = "indirect_vaccine_bench_visible_vaccine_definition_before_practice"
    indirect_visible_vaccine_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ]
    ...
    assert any(
        ...
        and case["setup_commands"] == ["go east", "go east"]
        and case["ai_interpretation"]["action"] == "explain"
        and case["ai_interpretation"]["target"] == "vaccine"
        and case["expected_success"] is True
        and case["expected_room_id"] == "vaccine_bench"
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 1
        for case in indirect_visible_vaccine_cases
    )


def test_learner_sentence_corpus_covers_vaccine_bench_visible_immune_definition_before_practice() -> None:
    target_case_id = "vaccine_bench_visible_immune_definition_before_practice"
    visible_immune_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ]
    ...
    assert any(
        ...
        and case["setup_commands"] == ["go east", "go east"]
        and case["expected_parser"]["action"] == "explain"
        and case["expected_parser"]["target"] == "immune"
        and case["expected_success"] is True
        and case["expected_room_id"] == "vaccine_bench"
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 1
        for case in visible_immune_cases
    )


def test_learner_sentence_corpus_covers_indirect_vaccine_bench_visible_immune_definition_before_practice() -> None:
    target_case_id = "indirect_vaccine_bench_visible_immune_definition_before_practice"
    indirect_visible_immune_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ]
    ...
    assert any(
        ...
        and case["setup_commands"] == ["go east", "go east"]
        and case["ai_interpretation"]["action"] == "explain"
        and case["ai_interpretation"]["target"] == "immune"
        and case["expected_success"] is True
        and case["expected_room_id"] == "vaccine_bench"
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 1
        for case in indirect_visible_immune_cases
    )
...
def test_learner_sentence_corpus_covers_item_room_encountered_only_bacteria_definition_after_leaving_source_room() -> None:
    target_case_id = (
        "item_room_encountered_only_bacteria_definition_after_leaving_source_room_rejected"
    )
    encountered_only_bacteria_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ]
    ...
    assert any(
        ...
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 0
        for case in encountered_only_bacteria_cases
    )


def test_learner_sentence_corpus_covers_indirect_item_room_encountered_only_bacteria_definition_after_leaving_source_room() -> None:
    target_case_id = (
        "indirect_item_room_encountered_only_bacteria_definition_after_leaving_source_room_rejected"
    )
    indirect_encountered_only_bacteria_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ]
    ...
    assert any(
        ...
        and case["setup_commands"] == ["go east", "go west"]
        and case["ai_interpretation"]["action"] == "explain"
        and case["ai_interpretation"]["target"] == "bacteria"
        and case["expected_success"] is False
        and case["expected_room_id"] == "research_camp"
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 0
        for case in indirect_encountered_only_bacteria_cases
    )


def test_learner_sentence_corpus_covers_item_room_encountered_only_strain_definition_after_leaving_source_room() -> None:
    target_case_id = (
        "item_room_encountered_only_strain_definition_after_leaving_source_room_rejected"
    )
    encountered_only_strain_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ]
    ...
    assert any(
        ...
        and case["setup_commands"] == ["go east", "go west"]
        and case["expected_parser"]["action"] == "explain"
        and case["expected_parser"]["target"] == "strain"
        and case["expected_success"] is False
        and case["expected_room_id"] == "research_camp"
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 0
        for case in encountered_only_strain_cases
    )
...
def test_learner_sentence_corpus_covers_practiced_item_room_strain_definition_after_leaving_source_room() -> None:
    target_case_id = "practiced_item_room_strain_definition_after_leaving_source_room"
    practiced_item_room_strain_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ]
    ...
    assert any(
        case["category"] == "accepted"
        and case["route"] == "deterministic_parser"
        and case["setup_commands"] == ["go east", "inspect strain", "go west"]
        and case["expected_parser"]["action"] == "explain"
        and case["expected_parser"]["target"] == "strain"
        and case["expected_success"] is True
        and case["expected_room_id"] == "research_camp"
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 1
        for case in practiced_item_room_strain_cases
    )


def test_learner_sentence_corpus_covers_indirect_practiced_item_room_strain_definition_after_leaving_source_room() -> None:
    target_case_id = (
        "indirect_practiced_item_room_strain_definition_after_leaving_source_room"
    )
    indirect_practiced_item_room_strain_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ]
    ...
    assert any(
        ...
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 1
        for case in indirect_practiced_item_room_strain_cases
    )
...
def test_learner_sentence_corpus_covers_indirect_practiced_item_room_bacteria_definition_after_leaving_source_room() -> None:
    target_case_id = (
        "indirect_practiced_item_room_bacteria_definition_after_leaving_source_room"
    )
    indirect_practiced_item_room_bacteria_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ]
    ...
    assert any(
        ...
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 1
        for case in indirect_practiced_item_room_bacteria_cases
    )


def test_learner_sentence_corpus_covers_item_room_visible_bacteria_definition_before_practice() -> None:
    target_case_id = "item_room_visible_bacteria_definition_before_practice"
    visible_bacteria_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ]
    ...
    assert any(
        ...
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 1
        for case in visible_bacteria_cases
    )


def test_learner_sentence_corpus_covers_indirect_item_room_visible_bacteria_definition_before_practice() -> None:
    target_case_id = "indirect_item_room_visible_bacteria_definition_before_practice"
    indirect_visible_bacteria_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ]
    ...
    assert any(
        ...
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 1
        for case in indirect_visible_bacteria_cases
    )


def test_learner_sentence_corpus_covers_item_room_visible_strain_definition_before_practice() -> None:
    target_case_id = "item_room_visible_strain_definition_before_practice"
    visible_strain_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ]
    ...
    assert any(
        ...
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 1
        for case in visible_strain_cases
    )


def test_learner_sentence_corpus_covers_indirect_item_room_visible_strain_definition_before_practice() -> None:
    target_case_id = "indirect_item_room_visible_strain_definition_before_practice"
    indirect_visible_strain_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ]
    ...
    assert any(
        ...
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 1
        for case in indirect_visible_strain_cases
    )
...
def test_learner_sentence_corpus_covers_practiced_start_room_organism_definition_after_movement() -> None:
    target_case_id = "practiced_start_room_organism_definition_after_movement"
    ...
    assert any(
        case["category"] == "accepted"
        and case["route"] == "deterministic_parser"
        and case["setup_commands"] == ["inspect organism", "go north"]
        and case["expected_parser"]["action"] == "explain"
        and case["expected_parser"]["target"] == "organism"
        and case["expected_success"] is True
        and case["expected_room_id"] == "fungus_grove"
...
def test_learner_sentence_corpus_covers_indirect_practiced_start_room_organism_definition_after_movement() -> None:
    target_case_id = (
        "indirect_practiced_start_room_organism_definition_after_movement"
    )
    indirect_practiced_start_room_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ]
    ...
    assert any(
        ...
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 1
        for case in indirect_practiced_start_room_cases
    )
...
def test_learner_sentence_corpus_covers_practiced_start_room_species_definition_after_movement() -> None:
    target_case_id = "practiced_start_room_species_definition_after_movement"
    practiced_start_room_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ...
    assert any(
        case["category"] == "accepted"
        and case["route"] == "deterministic_parser"
        and case["setup_commands"] == ["inspect species", "go north"]
        and case["expected_parser"]["action"] == "explain"
        and case["expected_parser"]["target"] == "species"
        and case["expected_success"] is True
        and case["expected_room_id"] == "fungus_grove"
...
def test_learner_sentence_corpus_covers_indirect_practiced_start_room_species_definition_after_movement() -> None:
    target_case_id = (
        "indirect_practiced_start_room_species_definition_after_movement"
    )
    indirect_practiced_start_room_cases = [
        case
        for case in load_corpus()
...
def test_learner_sentence_corpus_covers_indirect_practiced_start_room_evolve_definition_after_movement() -> None:
    target_case_id = (
        "indirect_practiced_start_room_evolve_definition_after_movement"
    )
    indirect_practiced_start_room_cases = [
        case
        for case in load_corpus()
...
def test_learner_sentence_corpus_covers_indirect_practiced_item_room_microscope_definition_after_leaving_source_room() -> None:
    target_case_id = (
        "indirect_practiced_item_room_microscope_definition_after_leaving_source_room"
    )
    indirect_practiced_item_room_cases = [
        case
        for case in load_corpus()
        if case["id"] == target_case_id
    ]
    ...
    assert any(
        ...
        and case["expected_state_unchanged"] is True
        and case["expected_vocabulary_request_count"] == 1
        for case in indirect_practiced_item_room_cases
    )
